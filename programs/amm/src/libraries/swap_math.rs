...
pub fn compute_swap_step(
    ...
    // Precompute fee denominator values once
    let fee_denom = FEE_RATE_DENOMINATOR_VALUE;
    let fee_numerator = fee_denom - fee_rate;

    if is_base_input {
        // Calculate amount after fees upfront
        let amount_remaining_less_fee = amount_remaining
            .mul_div_floor(fee_numerator.into(), fee_denom.into())
    ...
    // Cap output amount for exact output case
    if !is_base_input && swap_step.amount_out > amount_remaining {
        swap_step.amount_out = amount_remaining;
    }

    // Calculate fees - optimized logic
    swap_step.fee_amount = if is_base_input && !reached_target {
        // Dust case - remainder becomes fee
        amount_remaining.checked_sub(swap_step.amount_in)
            .ok_or(ErrorCode::CalculateOverflow)?
    } else {
        // Standard fee calculation
        swap_step.amount_in
            .mul_div_ceil(fee_rate.into(), fee_numerator.into())
            .ok_or(ErrorCode::CalculateOverflow)?
    };
