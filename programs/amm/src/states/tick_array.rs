        ...
        if zero_for_one {
            ...
            for i in (0..TICK_ARRAY_SIZE_USIZE).rev() {
                if self.ticks[i].is_initialized() {
                    return Ok(unsafe { self.ticks.get_unchecked(i) });
                }
            }
        } else {
    ...
    pub fn next_initialized_tick(
        ...
        if zero_for_one {
            ...
            while offset_in_array >= 0 {
                // Use unsafe get_unchecked for performance after bounds check
                if offset_in_array < TICK_ARRAY_SIZE {
                    let tick = unsafe { self.ticks.get_unchecked(offset_in_array as usize) };
