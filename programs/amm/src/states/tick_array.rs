...
impl TickArrayState {
    ...
    ///
    /// Takes `&self` (not `&mut self`) so the quote path's `&TickArrayState` can call it.
    /// The on-chain swap_internal dereferences the returned `&TickState` to an owned copy
    /// (TickState is `#[zero_copy]` → Copy), mutates the copy, then writes it back via
    /// `update_tick_state` — so the returned mutability was never used anyway.
    ...
    ///
    /// Takes `&self` for the same reason as `first_initialized_tick` above.
    pub fn next_initialized_tick(
        ...
        Ok(found_index.map(|i| &self.ticks[i as usize]))
    }
