...
pub fn swap_internal<'b, 'info>(
    ...
    let _updated_reward_infos = pool_state.update_reward_infos(block_timestamp as u64)?;
...
pub fn swap_on_swap_state(
    ...
    mut state: SwapState,
    tick_arrays_vec: &Vec<TickArrayState>,
    tickarray_bitmap_extension: &Option<TickArrayBitmapExtension>,
    amount_specified: u64,
    sqrt_price_limit_x64: u128,
    zero_for_one: bool,
    ...
    let mut tick_array_current = tick_arrays_vec
        .get(tick_array_index)
        .ok_or(ErrorCode::NotEnoughTickArrayAccount)?;
    while tick_array_current.start_tick_index != current_valid_tick_array_start_index {
        tick_array_index += 1;
        tick_array_current = tick_arrays_vec
            .get(tick_array_index)
            .ok_or(ErrorCode::NotEnoughTickArrayAccount)?;
    }
...
mod swap_test {
        ...
        let mut sum_amount_1: u64 = 0;
        {
            ...
            for position_param in position_params {
                ...
                if let std::collections::hash_map::Entry::Vacant(e) =
                    tick_array_map.entry(tick_array_lower_start_index)
                {
                ...
                if let std::collections::hash_map::Entry::Vacant(e) =
                    tick_array_map.entry(tick_array_upper_start_index)
                {
                    let mut tick_array_refcel = build_tick_array_with_tick_states(
                        pool_state.key(),
    ...
    mod sqrt_price_limit_optimization_min_specified_test {
        ...
        fn zero_for_one_base_input_with_min_amount_specified() {
            ...
            println!("sum_amount_0: {sum_amount_0}, sum_amount_1: {sum_amount_1}",);
...
mod benchmarks {
    ...
    fn benchmark_swap_scenario(amount: u64, expected_iterations: u32) {
        ...
        let _total_tick_lookup_time = std::time::Duration::ZERO;
        ...
        while swap_state.amount_specified_remaining > 0 && iterations < expected_iterations {
            ...
            swap_state.amount_specified_remaining = swap_state
                .amount_specified_remaining
                .saturating_sub(amount / expected_iterations as u64);
        ...
        println!(
            ...
            ((total_time - total_compute_time - total_math_time).as_nanos() as f64
                / total_time.as_nanos() as f64)
                * 100.0
        );
    }

    fn create_mock_tick_array(start_index: i32, _tick_spacing: u16) -> &'static TickArrayState {
    ...
    fn benchmark_swap_components(amm_config: &AmmConfig) {
        ...
        for _ in 0..iterations {
            let _ = U128::from(step_fee_amount).mul_div_floor(
                U128::from(amm_config.protocol_fee_rate),
        ...
        // Benchmark swap_math::compute_swap_step
        let start = Instant::now();
        for _ in 0..iterations {
            let _ = swap_math::compute_swap_step(
                sqrt_price_1,
    ...
    fn profile_compute_swap_step() {
        ...
        let total_time = start.elapsed();
        println!(
            "Total compute_swap_step time: {:?} per call",
        ...
        let start = Instant::now();
        for _ in 0..iterations {
            let _ = amount_remaining.mul_div_floor(
        ...
        println!("\nBottleneck analysis:");
        let accounted_time = fee_calc_time
            + delta_0_time
            + delta_1_time
            + sqrt_price_input_time
            + sqrt_price_output_time;
        println!("  - Total accounted time: {accounted_time:?}");
        println!(
            "  - Overhead/other: {:?}",
            (total_time / iterations).saturating_sub(accounted_time)
        );
    }
}
